...
function generateSpawnErrorMessage (isBuiltinNode) {
  if (isBuiltinNode) {
    `The server failed to start using Pulsar’s own version of node (version ${process.versions.node}).`;
  } else {
    `Make sure the path to your Node binary is correct and is of version 18 or greater.\n\nIf \`node\` is in your \`PATH\` and Pulsar is not recognizing it, you may set the path to your Node binary in this package’s settings. You may also opt into using Pulsar’s own bundled \`node\` (version ${process.versions.node}). Consult the README on the settings page for more information.`;
  }
}
...
class TypeScriptLanguageClient extends AutoLanguageClient {
  ...
  onSpawnError(err) {
    ...
    // Node path or version. But we should customize the error message
    // depending on whether the user has brought their own Node.
    let description = generateSpawnErrorMessage(
      atom.config.get(`${this.getPackageName()}.useBuiltinNode`)
    );
  ...
  writeJsconfigJsonToAtomHome() {
    const destinationPath = Path.join(process.env.ATOM_HOME, 'jsconfig.json');
    if (FS.existsSync(destinationPath)) {
      atom.notifications.addError(
        `${this.getPackageName()}: Failed to write jsconfig.json`,
        {
          description: `A \`jsconfig.json\` file already exists in your \`ATOM_HOME\` directory.`,
          dismissable: true
        }
      );
      return;
    ...
    let jsonString = JSON.stringify({
      "compilerOptions": {
        ...
        "module": "commonjs",
        "checkJs": false,
        "typeRoots": [`./packages/${this.getPackageName()}/node_modules/@types`],
        "types": ["atom"]
      },
      "include": ["init.js"]
    }, null, 2);
    FS.writeFileSync(destinationPath, jsonString, { encoding: 'utf8' });
  }

  promptForJsconfigJson() {
    const SHOULD_PROMPT_KEY = `${this.getPackageName()}.advanced.promptForJsconfigJson`;
    if (!atom.config.get(SHOULD_PROMPT_KEY)) {
      return;
    }

    let notification;
    const neverAskAgain = () => atom.config.set(SHOULD_PROMPT_KEY, false);

    const onClickYes = () => {
      this.writeJsconfigJsonToAtomHome();
      neverAskAgain();
    ...
    const onClickLater = () => notification?.dismiss();

    notification = atom.notifications.addInfo(
      `${this.getPackageName()}: Atom API autocompletion for init.js`,
      {
        dismissable: true,
        description: `This package can define a \`jsconfig.json\` in your \`ATOM_HOME\` directory in order to automatically make Pulsar API autocompletion and documentation available for when you edit your \`init.js\` file. Would you like to write this file to your \`ATOM_HOME\`?`,
        buttons: [
          { onDidClick: onClickYes, text: 'Yes, define a jsconfig.json' },
          { onDidClick: onClickNo, text: 'No, and don’t ask again' },
          { onDidClick: onClickLater, text: 'Remind me later' }
        ]
      }
    );
  }
  ...
  activate(...args) {
    // This package used to be named `pulsar-ide-typescript-alpha`, so we check
    // for old configuration defined at that package name just in case this is
    // the user's first time upgrading since the name change. Eventually we'll
    ...
    this.commandDisposable = atom.commands.add(
      'atom-workspace',
      {
        [`${packageName}:start-language-server`]: () => {
          ...
          // for non-TypeScript files. It only has that effect because this
          // command is defined in `activationCommands` in `package.json`.
          //
          // The main method of auto-activating this package is via
          // `activationHooks` — it'll activate whenever a user opens a
          // JavaScript or TypeScript file for editing.
          //
          ...
          // start a TypeScript language server if they're not actually editing
          // any TS/JS files, but I won't rule out the possiblity altogether.
          //
          ...
          // option is to provide automatic startup for TypeScript/JavaScript
          // files and ask the user to invoke startup for everything else.
        ...
        [`${packageName}:restart-language-server`]: async () => {
          ...
          try {
            ...
            atom.notifications.addSuccess(`Restarted typescript-language-server`);
          } catch (err) {
            console.error('Error restarting typescript-language-server');
            console.error(err);
            atom.notifications.addError(
              `Failed to restart typescript-language-server`,
              { description: err.message }
            )
          }
        },

        // Since this language server command is always available within a TS
        // or JS file, we might as well offer our own command so that it can be
        // assigned to a keybinding.
  ...
  getInitializeParams (projectPath, lsProcess) {
    let existing = super.getInitializeParams(projectPath, lsProcess);
    let memoryLimit = atom.config.get(`${this.getPackageName()}.advanced.maxTsServerMemory`);
    let advancedOptions;
    try {
      advancedOptions = JSON.parse(
        atom.config.get(`${this.getPackageName()}.advanced.initializationOptions`) ?? "{}"
      );
    } catch (err) {
      advancedOptions = {};
    }
    let initializationOptions = {
      hostInfo: `Pulsar v${atom.getVersion()}`,
      ...advancedOptions
    };
    if (memoryLimit > 0) {
      initializationOptions.maxTsServerMemory = memoryLimit;
    }

    existing.initializationOptions = initializationOptions;
    return existing;
  }
  ...
  mapConfigurationObject (config) {
    ...
    // TODO: `diagnostics.ignoredCodes` should be able to be specified here,
    // but past experiments have not been fruitful. Revisit.
    let bundle = {
  ...
  getPathToBin () {
    return Path.join(ROOT, 'node_modules', 'typescript-language-server', 'lib', 'cli.mjs');
  }

  startBuiltinNodeServerProcess () {
    let bin = this.getPathToBin();
    console.debug('Starting bin at path:', bin, 'using builtin Node');
  ...
  startServerProcess() {
    if (atom.config.get(`${this.getPackageName()}.useBuiltinNode`)) {
      return this.startBuiltinNodeServerProcess();
    }
  ...
  shouldIgnoreLinterMessage(diagnostic, editor, _range) {
